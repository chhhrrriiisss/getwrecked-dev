...
Additional modifications:
	...
	- No longer removes attached static/groundweaponholder objects
	by Sli
...
_deadMenLimit = 6;								// Bodies. Set -1 to disable.
_deadMenDistCheck = TRUE;						// TRUE to delete any bodies that are far from players.
_deadMenDist = 300;							// Distance (meters) from players that bodies are not deleted if below max.
...
_deadVehicleDist = 300;						// Distance (meters) from players that destroyed vehicles are not deleted if below max.
_craterLimit = 10;								// Craters. Set -1 to disable.
_craterDistCheck = TRUE;						// TRUE to delete any craters that are far from players.
_craterDist = 300;								// Distance (meters) from players that craters are not deleted if below max.
...
_staticsDist = 500;							// Distance (meters) from players that static weapons are not deleted if below max.
...
for "_i" from 0 to 1 step 0 do {
	...
	if (!deleteManagerPublic) exitWith {};

	diag_log format['Cleanup running at %1', diag_tickTime];
	...
	if (!(_weaponHolderLimit isEqualTo -1)) then {
		if ((count (allMissionObjects "WeaponHolder")) > _weaponHolderLimit) then {
			while {(((count (allMissionObjects "WeaponHolder")) - _weaponHolderLimit) > 0)} do {
				_o = ((allMissionObjects "StaticWeapon") select 0);
				if (isNull attachedTo _o) then {
					deleteVehicle _o;
				};
