...
if (!isNil "GW_CURRENTZONE") then {
	...
	_inZone = if !(call inRace) then {

		// Assume out of zone if bad data
		if (count GW_CURRENTZONE_DATA == 0) exitWith { false };

		// Not in an active race? Use default zone data
		([GW_CURRENTPOS, GW_CURRENTZONE_DATA] call checkInZone)

	} else {

		// If we're in a race, make sure we're at least 1km away from the workshop
		(GW_CURRENTPOS distance GW_WORKSHOP > 1000)
