...
_nearby = _position nearEntities [GW_VEHICLE_CLASSES, 8];
...
_maxWeapons = (_data select 3) select 1;
_maxModules = (_data select 3) select 2;
...
_modifier = if (!isNil "_data") then { (((_data select 3) select 0) select 0) } else { 1 };
_maxMass = if (!isNil "_data") then { (((_data select 3) select 0) select 1) } else { 99999 };
