...
// Clear/Unsimulate unnecessary items near workshop
{
	...
	if ( (isPlayer _x || (_x call isVehicle)) && !_i) then { _x enableSimulation true; } else { _x enableSimulation false; };
	false
...
checkUIEvents = {
	if (GW_CURRENTZONE == "workshopZone" && (_this select 3) in ['GetInDriver', 'GetInTurret', 'GetInGunner', 'GetInCargo']) exitWith { true };
	if ((_this select 3) in ['DisAssemble', 'TakeWeapon', 'TakeItem', 'Gear']) exitWith { true };
	false
};

inGameUISetEventHandler ["Action", "_this call checkUIEvents"];
