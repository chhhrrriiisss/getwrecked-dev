...
GW_EVENTS_LIST = [
	...
	[
...

		{
			...
			// Checks manifest for dead players (used for players who decide to quit suddenly so the DC EH to remove them from zone isn't run)\
			_update = false;
			{
			...
			if (_update) then { publicVariable "GW_ZONE_MANIFEST"; };
		},
