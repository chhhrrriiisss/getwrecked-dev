...
GW_VEHICLE_CLASSES = ["Car", "Tank", "Bicycle"];
...
// Custom/modded compile
call compile preprocessFile 'global\custom.sqf';
...
inRace = {
	if (isNil "GW_CURRENTRACE") exitWith { false };
...
isVehicle = {
	if ({
		if (_this isKindOf _x) exitWith { 1 };
		false
	} count GW_VEHICLE_CLASSES > 0) exitWith { true };
	false
};
...
allVehicles = {
	_vs = [];
	{
		_vs append (allMissionObjects _x);
		false
	} count GW_VEHICLE_CLASSES;
	_vs
};
...
GW_WORKSHOP = getMarkerPos "workshopZone_camera";
